    ...
    try:
        ...
        if ENVIRONMENT == "production" or FORCE_VALIDATION:
            ...
            if not x_twilio_signature:
                logger.warning(
                    "No X-Twilio-Signature header found in WebSocket request"
                )
            ...
            if not validation_successful:
                logger.error(
                    "Signature validation failed for all URLs",
                    {
                        "attempted_urls": validation_urls,
                        "signature": x_twilio_signature[:20] + "..."
            ...
            logger.info(
                "WebSocket signature validated", {"validated_with": successful_url}
            )
        else:
        ...
        logger.info(
            ...
            {"url": str(websocket.url), "session_ready": True},
        )
        ...
        logger.error(
            "WebSocket connection error", {"error": str(e), "url": str(websocket.url)}
        )
        try:
            await websocket.close(code=1011, reason="Server error")
        except Exception as e:
            logger.error(
                "WebSocket close error", {"error": str(e), "url": str(websocket.url)}
            )
            pass
